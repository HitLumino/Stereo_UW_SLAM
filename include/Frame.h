...
public:
    ...
    ///lsd_line提取
    int ExtractLSD(const cv::Mat &im1, const cv::Mat &im2);
...
public:
    ...
    //std::vector<KeyLine> mvGoodKeylines;///匹配好了的线特征
    //std::vector<KeyLine> mvGoodKeylinesRight;///匹配好了的线特征
    ...
    /// \brief mvKeylines
    std::vector<KeyLine> mvKeylines;///当前帧的线特征
    std::vector<KeyLine> mvKeylinesRight;///当前帧的线特征
    std::vector<cv::Point2f> mLine1StartPoints;
    std::vector<cv::Point2f> mLine2StartPoints;
    std::vector<cv::Point2f> mLine1EndPoints;
    std::vector<cv::Point2f> mLine2EndPoints;
    std::vector<float> mvLineStartDepth;
    std::vector<float> mvLineEndDepth;

    std::vector<cv::Mat> mLineStartPoints_3;
    std::vector<cv::Mat> mLineEndPoints_3;
    cv::Mat mlinedescriptors;
    cv::Mat mlinedescriptorsRight;
    ////
