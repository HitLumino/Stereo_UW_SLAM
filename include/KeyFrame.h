...
public:
    ...
    // Pose functions
    // 这里的set,get需要用到锁
    //void ExtractLSD(const cv::Mat &im1,const cv::Mat &im2);
...
public:
    ...
    //std::vector<KeyLine> mvKeylinesRight;
    ...
    const std::vector<cv::Point2f> mLine1StartPoints;
    const std::vector<cv::Point2f> mLine2StartPoints;
    const std::vector<cv::Point2f> mLine1EndPoints;
    const std::vector<cv::Point2f> mLine2EndPoints;
    const std::vector<float> mvLineStartDepth;
    const std::vector<float> mvLineEndDepth;

    const std::vector<cv::Mat> mLineStartPoints_3;
    const std::vector<cv::Mat> mLineEndPoints_3;

    const cv::Mat mlinedescriptors;///线描述子
    const cv::Mat mlinedescriptorsRight;///右线描述子
    ///
