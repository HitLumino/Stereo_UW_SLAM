...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    ///新成员变量
    mLine1StartPoints(F.mLine1StartPoints),mLine2StartPoints(F.mLine2StartPoints),
    mLine1EndPoints(F.mLine1EndPoints),mLine2EndPoints(F.mLine2EndPoints),
    mvLineStartDepth(F.mvLineStartDepth),mvLineEndDepth(F.mvLineEndDepth),
    mLineStartPoints_3(F.mLineStartPoints_3),mLineEndPoints_3(F.mLineEndPoints_3),
    mlinedescriptors(F.mlinedescriptors.clone()),mlinedescriptorsRight(F.mlinedescriptorsRight.clone()),
...
cv::Mat KeyFrame::UnprojectStereoLineStart( int i)
{
    ...
    //vector<float> m1(mvLineEndDepth);
    if(z>0)
    {
        const float x = (u-cx)*z*invfx;
        const float y = (v-cy)*z*invfy;
...
//vector<cv::Mat> KeyFrame::GetWorldLinePoints()
//{
